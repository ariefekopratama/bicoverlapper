...
public void setup() {
  ...
  System.out.println("La caja en "+xTopOverviewBox+" siendo el tamaño "+screenWidth);
...
public void resize(int w, int h)
	...
	//size(screenWidth,screenHeight);//Sólo en caso de usarlo desde el panel, el P3D parece dar problemas
	//size(screenWidth,screenHeight,P3D);//TODO: Sólo en caso de usarlo desde el panel, el P3D parece dar problemas
	//size(screenWidth,screenHeight,OPENGL);//Sólo en caso de usarlo desde el panel, el P3D parece dar problemas
	 //Overview
	...
	//  yTopMagnifier = yTopOverviewBox+(h-hant);
	  ...
	  //TODO: Hay que afinar más dónde quedan xTop e yTopMagn por el incremento en tamaño que tienen overviewBox
	//	offsetX = (xTopOverviewBox - xTopMagnifier) * (1 / factor); 
	//	offsetY = (yTopOverviewBox - yTopMagnifier) * (1 / factor); 
...
protected void mouseMoved() {
  if (g.getDragNode() == null) 
    ...
    if(g.getHoverNode()==null)//Search for hover hull
    	{
    	for(int i=0;i<this.numClusters;i++)
    		{
    		MaximalCluster mc=this.getClusterInPos(i);
    		if(mc!=null && mc.hull!=null && mc.hull.contains(xpress, ypress))	
    				{
    				g.getHoverClusters().put(mc.label, mc);
    				System.out.println("Estamos en cluster "+i);
    				}
    		}
    	}
  }
...
//			System.out.println("Añadiendo cluster de tamaño "+lista.size());
	...
	for (int l = 2; l < data.length; l++)
		...
		if (dataToken.length >1)
			...
			if (dataToken.length >1)
				...
				System.out.println("Añadiendo cluster "+cont+" de tamaño "+lista.size());
				//System.out.println("---------------------------");
				clusters.put("cluster"+cont, lista);
				this.numClusters++;
				cont++;
...
Graph buildOrderedCompleteGraph()
{
  ...
  removeNonOverlapped();
  System.out.println("No solapados tenemos "+numClusters);
  MaximalCluster[] clu=orderClusters();//Añade un campo orden a cada cluster
...
void removeNonOverlapped()
	...
	for(int i=0;i<clustersToRemove.size();i++)
		...
		for(int k=0;k<lista.size();k++)//Para cada nodo del cluster
			...
			if(connectionThreshold>0)
				{//Tenemos que comprobar que no quitamos un nodo que está en otros clusters
				//TODO: No sé si es mejor dejar el nodo con la información de que pertenece a varios clusters o quitarla también
...
void removeNonSelected(BiclusterSelection bs)
	...
	for(int i=0;i<clustersToRemove.size();i++)//Para cada cluster a quitar
		...
		for(int k=0;k<lista.size();k++)//Para cada nodo del cluster
			...
			if(toRemove)	//se quita solo si no está en algún cluster seleccionado	
				{
				nodesToRemove.add(nr);
				g.getNodes().remove(nr.getLabel());//y a sí mismo
				}
...
void restoreClusters()
	...
	if(edgesToRemove!=null)
		for(int i=0;i<edgesToRemove.size();i++)
			...
			SpringEdge ret=(SpringEdge)g.getEdges().get(e.getTo().getLabel()+"->"+e.getFrom().getLabel());	//MIramos a ver si está en un sentido
...
public void buildGraph()
	...
	insertDetails();
	Runtime.getRuntime().gc();//Al final del constructor quizás sea buena idea
	}
...
Graph buildCompleteGraph() 
	{
	 ...
	 // if(titles==null && getTitleFile()!=null)	titles=loadStrings(getTitleFile());
	  ...
	  println("número de nodos: "+g.getNumNodes()+", con centrales "+g.getNumCenterNodes()+" y aristas "+g.getNumEdges());
...
//----------------------------- GETTERS AND SETTERS ------------------------------
/*
 * TODO: Muchos de estos parámetros, aunque están en biclusVis, hay que asegurar que se usan más en
