...
import es.usal.bicoverlapper.kernel.managers.ConfigurationListener;
import es.usal.bicoverlapper.kernel.managers.ConfigurationMenuManager;
import es.usal.bicoverlapper.utils.ColorChooser;
...
public abstract class Diagram extends JPanel {
	...
	JPanel getPanelAnclajes(Session sesion, ConfigurationMenuManager gestor){
		...
		for(int i = 1; i < (ventanasH.size()+1); i++) {
			...
			if(!ventana.getTitle().equals(this.getWindow().getTitle()) && !es.usal.bicoverlapper.utils.ArrayUtils.contains(this.getWindow().getHooks(),ventana.getTitle()))
