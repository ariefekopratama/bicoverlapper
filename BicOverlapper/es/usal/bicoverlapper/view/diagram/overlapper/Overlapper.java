...
public class Overlapper extends JProcessingPanel implements GeneRequester {
	...
	public void setup() {
		...
		// Handles
		/*
		 * handles = new Handle[handleNum]; for(int i=0; i<handleNum; i++)
		 * handles[i] = new Handle(handleSize*2, (handleSize*2)*i +handleSize, 0
		 * , handleSize, handles, this);
		 * 
		 * handles[0].setLabel("Repulsion"); handles[0].setMinValue(G);
		 * handles[0].setMaxValue(G+(handleLength*stepG));
		 * handles[0].setCurrentValue(G); handles[0].moveHandle(G);
		 * 
		 ...
		 * 
		 * handles[2].setLabel("Edge Size"); handles[2].setMinValue(edgeLength);
		 ...
		 * 
		 * handles[3].setLabel("Expansion"); handles[3].setMinValue(K);
		 * handles[3].setMaxValue(K+(handleLength*stepK));
		 * handles[3].setCurrentValue(K); handles[3].moveHandle(K);
		 */
	...
	public void resize(int w, int h) {
		...
		// TODO: Hay que afinar más dónde quedan xTop e yTopMagn por el
		// incremento en tamaño que tienen overviewBox
		// offsetX = (xTopOverviewBox - xTopMagnifier) * (1 / factor);
		// offsetY = (yTopOverviewBox - yTopMagnifier) * (1 / factor);
