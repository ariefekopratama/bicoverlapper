...
public class BubblesDiagram extends Diagram {
	 ...
	 */
	/*
	 * public void repaint() { if(v!=null && sesion!=null &&
	 * sesion.getSelectedBicluster()!=null) { //v.cancel("animatePaint");
	 * v.run("color"); //v.run("animatePaint"); v.run("layout"); // start up the
	 * animated layout v.run("filter"); v.run("animate");
	 * 
	 * } }
	 */
	...
	public void createAxisLayout() {
		...
		nodeStroke = new NodeStrokeAction("graph.nodes",
				...
				sesion.getSearchColor());// En el futuro, tb goodness TODO:
											// linear scale no es la mejor
											// manera si son círculos
		ItemAction strokeWidth = new StrokeAction("graph.nodes",
				new BasicStroke(3));
		...
		d.addControlListener(new ControlAdapter() {
			public void itemEntered(VisualItem item, MouseEvent e) {
				if (item.canGetString("name"))
					...
					// +"_"+item.getString("id")+" "+item.getString("width")+"x"+item.getString("height"));
	...
	private static class EdgeColorAction extends ColorAction {
		...
		EdgeColorAction(String group) {
			...
			// super(group, sel, Constants.NUMERICAL, VisualItem.FILLCOLOR,
			// palette);
			// add("_hover", ColorLib.rgba(0,0,255,229));
			// add("ingroup('_search_')", ColorLib.rgba(0,255,0,250));
			// add("ingroup('_focus_')", ColorLib.rgba(255,0,255,229));//Pinta
			// morado si pinchamos en un nodo
		}
	} // end of inner class EdgeColorAction

	static class NodeSizeAction extends DataSizeAction {
		NodeSizeAction(String group, String sel, int bins, int scale) {// Hacemos
																		// el
																		// tamaño
																		// en
																		// función
	...
	static class NodeColorAction extends ItemAction// extends DataColorAction
	{
		...
		NodeColorAction(String group, String sel, String sel2, int[] palette,
			...
			/*
			 * while(it.hasNext()) { VisualItem item=(VisualItem)it.next();
			 * boolean add=true; if(tipo.size()>0) { for(int
			 * i=0;i<tipo.size();i++)
			 * if((item.getString(sel)+" "+item.getInt("width"
			 * )+"x"+item.getInt("height")).equals(tipo.get(i))) {add=false;
			 * break;} } if(add)
			 * {tipo.add(item.getString(sel)+" "+item.getInt("width"
			 * )+"x"+item.getInt("height"));} }
			 */
		}
	...
	static class NodeStrokeAction extends ColorAction {
		NodeStrokeAction(String group, Color hc, Color selc, Color sc) {
			super(group, VisualItem.STROKECOLOR, ColorLib.gray(0, 0));// Sin
																		// borde
																		// por
																		// defecto
			add("_hover",
			...
			add("ingroup('_focus_')",
					ColorLib.getColor(selc.getRed(), selc.getGreen(),
							selc.getBlue(), selc.getAlpha()).getRGB());// Pinta
																		// morado
																		// si
																		// pinchamos
																		// en un
																		// nodo
			add("ingroup('_bicluster_')", ColorLib.gray(0));// Pinta morado si
															// pinchamos en un
															// nodo
		}
	} // end of inner class NodeColorAction
