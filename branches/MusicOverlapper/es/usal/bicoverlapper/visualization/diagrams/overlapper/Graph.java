		...
		if(bv.isDrawDual())
	  	...
	  	if(hoverNode!=null && !selectedNodes.containsKey(hoverNode.label))
		    	...
		    	if(bv.isAbsoluteLabelSize())	bv.textSize(ls+1);
	  	    	...
	  	    	//bv.fill(0,0,0,255);
		    	Color chn=bv.paleta[Overlapper.hoverNodeLabelColor];
...
public void drawHoverHull()
	  ...
	  while(itGraph.hasNext())
		...
		if(n.clusters.size()>=bv.nodeThreshold && !nodeExcluded(n))
	    	...
	    	Color cc=bv.paleta[Overlapper.conditionLabelColor];
	 		if(n.isGene())	bv.fill(cg.getRed(), cg.getGreen(), cg.getBlue(), 255);
	    	else			bv.fill(cc.getRed(), cc.getGreen(), cc.getBlue(), 255);	
	    	bv.text(n.getLabel(), (float)n.getX(), (float)n.getY()-n.getHeight());
   ...
   *
   *The nº of possible misplacements is sum(numClusters-numClustersInWhichNodeKis)
   */
 ...
 */
/*
 * Versión 1 iteración para cada nodo, 1 paso
...
*/
//Versión 1 iteración para cada nodo -> todos los pasos (de momento parece más rápida y da mejores resultados)
public void fastHillClimberRelocation(double jump, double coolStep, boolean permissive, int[] metrics, double[] weights)
	...
	System.out.println("Improvement finished!: "+(end-start)/1000+" s");
	//System.out.println("Error tras corrección "+this.getFailedPositionMetric());
	System.out.println("Error tras corrección "+this.getMetric(metrics, weights));
	}
...
public void intelligentFastHill()
	{
	//Voy a probar simplemente a mover a la zona a la que pertenecen a cada nodo que esté mal posicionado
	double start=System.currentTimeMillis();
	double itFail=this.getFailedPositionMetric();
	...
	for(int i=0;i<p1.xpoints.length-1;i++)//Compute the intersection points of lines
		...
		for(int j=0;j<p2.xpoints.length-1;j++)
			...
			if(den!=0)
				...
				//System.out.println("punto de intersección: "+xi+", "+yi);//OJO, da el punto prolongando las líneas!
				if(xi>=minx1 && xi>=minx2 && xi<=maxx1 && xi<=maxx2 && yi>=miny1 && yi>=miny2 && yi<=maxy1 && yi<=maxy2 )
					{
					//System.out.println("Es punto realmente de intersección de los segmentos");
...
public static Polygon intersect(Polygon p1, Polygon p2)
	...
	System.out.println("Polígono de "+xp.countBorderSegments()+" segmentos");
...
public static Polygon intersect(Polygon p1, Polygon p2)
	...
	Polygon pi=new Polygon();
	//Overlapper ov=this.getApplet();
	ArrayList<Integer> lineas1=new ArrayList<Integer>();//Identificador de vértice de inicio de línea en las que se producen las intersecciones 
...
public void fastHillClimber2()
	...
	while(it.hasNext())
		...
		if(nodeFail>0)//De momento sólo para los nodos fallidos, que si no puede ser mucha carga
			{
			System.out.println("Mejorando "+n.label+", en posición "+n.getX()+", "+n.getY());
			...
			System.out.println("El segmento más cercano no incluido es el que va de "+minLine.getX1()+", "+minLine.getY1()+" a "+minLine.getX2()+", "+minLine.getY2()+"; con distancia"+minDist+"; del cluster "+minC.label);
			...
			if(pi!=null)
				{
				System.out.println("El punto de intersección con prolongación de "+x0+", "+y0+" es "+pi.x+", "+pi.y);
				if(x0<pi.x)	n.setX(pi.x+1);
				else		n.setX(pi.x-1);//TODO: esto no es muy exacto, debería ser en oblicuo
				if(y0<pi.y)	n.setX(pi.y+1);
				else		n.setX(pi.y-1);//TODO: esto no es muy exacto, debería ser en oblicuo
				draw(priorities);
...
double end=System.currentTimeMillis();
System.out.println("Improvement finished!: "+(end-start)/1000+" s");
System.out.println("Error tras corrección (dual) "+this.getFailedPositionMetricDual());
}
			...
			if(dn!=dn2)
				...
				while(itbic.hasNext())
					...
					if(dn2.clusters.containsValue(c))//Coinciden, así que creamos una arista entre ellos	
						...
						//e.k=this.getApplet().getStiffness()*dn.subNodes.size()*dn2.subNodes.size();
						e.k=this.getApplet().getStiffnessFactor()*clustersInCommon*(dn.subNodes.size()+dn2.subNodes.size());
		...
		while(it3.hasNext())//for each other node
			...
			if(dn!=dn2)
				...
				if(dn.adjacent(dn2))	//add edge
					...
					SpringEdge e=new SpringEdge(dn, dn2);
					e.setGraph(this);
					e.nl=this.getApplet().getEdgeLengthFactor()+(dn.getGroupRadius()+dn2.getGroupRadius());
					e.k=this.getApplet().getStiffnessFactor()*dn.subNodes.size()*dn2.subNodes.size();
					System.out.println("Longitud para edge de "+e.nl+", radios de "+dn.getGroupRadius()+" y "+dn2.getGroupRadius());
					dualEdges.put(e.hashCode(), e);
					}
				}
...
void computeMaxZones()
	...
	System.out.println("Hay un máximo de "+maxZones+" zonas posibles de un total de "+max+" para hacer superficies");
	/*
	Iterator<DualNode> it=dualNodes.values().iterator();
	maxZones=0;
	while(it.hasNext())
