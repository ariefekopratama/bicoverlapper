 ...
 */
/**
 * Class with genes and conditions selected, as lists of integers.
 * In case that only a genes are selected. You can choose to leave the condition
...
public class BiclusterSelection {
        ...
        public BiclusterSelection(LinkedList<Integer> g, LinkedList<Integer> c)
                ...
                genes=g;
                conditions=c;
                //Eliminamos el primero de una de las dos si vale cero, para evitar la mierdilla de las listas
                if(genes.size()>0      && genes.get(0)==-333)                   genes.remove(0);
                if(conditions.size()>0 && conditions.get(0)==-333)              conditions.remove(0);
         ...
         * @param genes LinedList<Integer> of gene ids selected
         */
        public void setGenes(LinkedList<Integer> genes) {
                this.genes = genes;
                if(this.genes.size()>0 && this.genes.get(0)==-333)                      this.genes.remove(0);
        }
         ...
         * @param conditions    LinedList<Integer> of condition ids selected
         */
        public void setConditions(LinkedList<Integer> conditions) {
                this.conditions = conditions;
                if(this.conditions.size()>0 && this.conditions.get(0)==-333)                    this.conditions.remove(0);
        }
