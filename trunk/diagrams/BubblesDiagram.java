...
/**
 * This Diagram gets a set of biclusters an represents them as transparent, colored, maybe overlapped circles.
 * Radius of the circles are determined by the size in expression levels of the biclusters. Transparency is given by the homogeneity of the bicluster (STILL IN DEVELOPMENT)
 * Position is given by considering gene and conditions as multidimensional points and projecting them to 2D by some multidimensional
 * technique (@see createAxisLayout for more information of the projection implemented at this moment)
...
public class BubblesDiagram extends Diagram {
	...
	private String[] textoLabel = {"Selection", "Search","Hover", "Set 1", "Set 2", "Set 3","Background"};
	...
	/**
	 * Session constructor
	 * @param session	Session layer linked to this diagram
	 * @param dim		Default dimension for this diagram
	 */
...
//		Miramos qué tenemos que hacer con posibles selecciones de biclusters
		if(v!=null && sesion!=null && sesion.getSelectedBicluster()!=null)	//version modo OR
				...
				for(int i=0;i<bd.getGraph().getNodeCount();i++)
					...
					for(int j=0;j<lista.size();j++)
						...
						if(lgn.contains(lista.get(j)))	
								...
								currentBiclusters.addItem(item);
								added=true;
								break;
								}
						}
	...
	public int getId(){
		return kernel.Configuration.BUBBLE_MAP_ID;
	}

	/**
	 * Constructs all prefuse internal structure to represent a bubble diagram with the following projection.
	 * 1) Take the conditions, in order, as a m-dimensional point c1,c2, ...,cm.
	 * 2) Take the genes, in order, as a n-dimensional point g1,g2, ...,gn.
	 * 3) Determine each bicluster (Bk) points Ck and Gk, where Ck=ck1,...ckm, with cki=1 if condition i is in the 
	 * bicluster Bk and 0 otherwise (the same for Gk and genes).
	 * 4) Multiply Ck and Gk by CC=(1,2,3...,m) and GG=(1,2,3,...,n).
	 * 5) An divide the results by m and n, respectively. 
	 * 6) Take the following results as x (Gk) and y (Ck). 
	 * 7) Draw the corresponding bubble at (x,y).
	 */
	public void createAxisLayout()
		...
		currentBiclusters=new BubbleFocusControl(sesion,"filter", Visualization.FOCUS_ITEMS, v);
	...
	public void endConfig(){
		...
		sesion.setBicSet1Color(new Color(paleta[BubblesDiagram.bicColor1].getRed(), paleta[BubblesDiagram.bicColor1].getGreen(), paleta[BubblesDiagram.bicColor1].getBlue(),100));
		sesion.setBicSet2Color(new Color(paleta[BubblesDiagram.bicColor2].getRed(), paleta[BubblesDiagram.bicColor2].getGreen(), paleta[BubblesDiagram.bicColor2].getBlue(),100));
		sesion.setBicSet3Color(new Color(paleta[BubblesDiagram.bicColor3].getRed(), paleta[BubblesDiagram.bicColor3].getGreen(), paleta[BubblesDiagram.bicColor3].getBlue(),100));
	...
	public void run()
	 ...
	 static class EdgeColorAction extends ColorAction 
	 	{
		static	int[] palette = new int[]{ ColorLib.gray(0,123), ColorLib.gray(100,123), ColorLib.gray(200,123) };
...
//			super(group, sel, Constants.NUMERICAL, VisualItem.FILLCOLOR, palette);
	           				         //add("_hover", ColorLib.rgba(0,0,255,229)); 
...
//	          add("ingroup('_focus_')", ColorLib.rgba(255,0,255,229));//Pinta morado si pinchamos en un nodo
		...
		NodeSizeAction(String group, String sel, int bins, int scale) 
	     	{//Hacemos el tamaño en función del número de genes y condiciones que contenga
	    ...
	    static class NodeColorAction extends ItemAction//extends DataColorAction 
			...
			NodeColorAction(String group, String sel, String sel2, int[] palette, Visualization vis) 
	        	{
	            super(group);
		        ...
		        while(it.hasNext())
		          	{
		           	...
		           	if(tipo.size()>0)
		           		...
		           		for(int i=0;i<tipo.size();i++)
		           			if(item.getString(sel).equals(tipo.get(i)))	{add=false; break;}
		           		}
	...
	static class NodeShapeAction extends ShapeAction 
 		{
		NodeShapeAction(String group, int shape) 
	     	{//Hacemos el tamaño en función del número de genes y condiciones que contenga
	         super(group, shape);
	 		}
    ...
    static class NodeStrokeAction extends ColorAction 
		...
		NodeStrokeAction(String group, Color hc, Color selc, Color sc) 
            ...
            add("ingroup('_bicluster_')", ColorLib.gray(0));//Pinta morado si pinchamos en un nodo
        	}
    	} // end of inner class NodeColorAction
