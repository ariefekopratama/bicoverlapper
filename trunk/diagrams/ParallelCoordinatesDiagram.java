...
import data.MultidimensionalData;
...
//	*** Buffer especial para optimización en el dibujado de las líneas de fondo
	...
	private int[] seleccionPuntos = {kernel.Configuration.PARALLEL_COORDINATES_ID};// kernel.Configuration.DiagramaPuntosId, kernel.Configuration.BubbleGraphId,
...
/*	private static final int colorEtiquetaVar = 0;
	...
	private static final int colorBicluster= 12;
//	private Color[] paleta = {Color.BLUE, Color.RED, Color.YELLOW, Color.BLACK, new Color(200,200,200), Color.BLUE,
//	  Color.YELLOW, Color.BLACK, Color.RED, Color.WHITE, Color.BLACK, Color.BLACK};
//	private Color[] paleta = {Color.DARK_GRAY, new Color(0,0,255,100), Color.YELLOW, Color.DARK_GRAY, new Color(200,200,200), Color.LIGHT_GRAY,
//	  Color.YELLOW, Color.BLACK, Color.RED, Color.WHITE, Color.LIGHT_GRAY, Color.LIGHT_GRAY, new Color(200,0,0,100)};
	private String[] textoLabel = {Translator.instance.configureLabels.getString("s11"), Translator.instance.configureLabels.getString("s4"),
			...
			Translator.instance.configureLabels.getString("s22"), Translator.instance.configureLabels.getString("s37"),
			"Bicluster Color"};*/
	...
	public ParallelCoordinatesDiagram(Session sesion, Dimension dim){
		...
		if(System.getProperty("os.name").contains("indows"))
			{
			iconoScrollUp = "src\\recursos\\up.png";
			iconoScrollDown = "src\\recursos\\down.png";
			iconoScrollSelecUp = "src\\recursos\\upSelec.png";
			iconoScrollSelecDown = "src\\recursos\\downSelec.png";
			}
		else
			{
			iconoScrollUp = "src/recursos/up.png";
		...
		//c=c.getGoodColor(sesion.getNumVentanas()+sesion.getNumColores()-sesion.getNumVentanaBubbleGraph());
		/*c=c.getGoodColor(0);
		colorSeleccion=new Color(c.getR(), c.getG(), c.getB());
		this.paleta[colorVarSelec]=colorSeleccion;
		this.paleta[colorLinea]=colorSeleccion;
			*/	
...
//		 Establecemos las opciones de rendering y antialiasing de la gráfica
	...
	private void iniciarAtributos(){
		...
		this.maxVar = new double[datos.getNumFields()];
		this.minVar = new double[datos.getNumFields()];
		this.maxText = new double[datos.getNumFields()];
		this.minText = new double[datos.getNumFields()];
		this.ratio = new double[datos.getNumFields()];
		
		this.currentTextInf = new double[datos.getNumFields()];
		this.currentTextSup = new double[datos.getNumFields()];
		this.valorInf = new double[datos.getNumFields()];
		this.valorSup = new double[datos.getNumFields()];
		
		// iniciamos los atributos del cambio de variables
		this.ejesVars = new Line2D.Double[datos.getNumFields()];
		this.ordenVars = new int[datos.getNumFields()];
		...
		this.scrollSup = new Rectangle2D.Double[datos.getNumFields()];
		this.scrollInf = new Rectangle2D.Double[datos.getNumFields()];
		this.cotaSup = new double[datos.getNumFields()];
		this.cotaInf = new double[datos.getNumFields()];
	...
	private void calcularAtributos(){
		...
		if(sesion.dataLoaded()){
			...
			intervaloVar = longEjeX/(datos.getNumFields()-1);
			...
			for(int i = 0; i < datos.getNumFields(); i++){
				...
				for(int j = 0; j < datos.getNumTuples(); j++){
	...
	private void drawEtiquetas(Graphics2D g2) {
		...
		if(scrollFijado && (varScroll != -1)){
			...
			String cad=sesion.getData().format(valor, varScroll);
		...
		// representamos los valores de referencia de la escala
		{
			...
			String cad=sesion.getData().format(valor, 0);
			...
			valor = minText[0];
			cad=sesion.getData().format(valor, 0);
			...
			if(ejesRelativos)
				{
				for(int i = 1; i < datos.getNumFields(); i++)
					{
					valor = maxText[i];
					cad=sesion.getData().format(valor, i);
	...
	{
	for(int j=0;j<datos.getNumFields()-1;j++)
		{
	...
	private void drawLineas(Graphics2D g2) {
		...
		//System.out.println("actualizar tuplas es "+actualizarTuplas);
		if(actualizarTuplas)
			{
			for(int i = 0; i < datos.getNumTuples(); i++)
				{
				for(int j = 0; j < (datos.getNumFields()-1); j++)
					{
		...
		//System.out.println("1) Preparación de tuplas "+(t2-t1)/1000);
		...
		if(selecPuntos!=null)	System.out.println("Dibujando las líneas, tenemos "+selecPuntos.getNumTuples());
		else	System.out.println("No hay biclusters!!!");
		
		//------------------------------- preparación de las líneas con el gp
		
		int step=1;
		int numLineasFondo=0;
		if(datos.getNumTuples()>maxLineas)	step=datos.getNumTuples()/maxLineas;
				...
				if(tuplaSeleccionada!=i)
					...
					if(selecPuntos!=null && selecPuntos.getNumTuples()>0 && (selecPuntos.isSelectedX(i) || selecPuntos.getElementY(i)) )
						...
						if(selecBic==null)
							...
							if(nc<datos.getNumFields()-1)
								{
								gpLineasSelecBic.append(getLine(i,0, nc-1),false);
								int init=nc-1;
								if(init<0)	init=0;
								gpLineasSelec.append(getLine(i,init, datos.getNumFields()-1), false);
								}
							else
								{
								for(int j = 0; j < datos.getNumFields()-1; j++)
		...
		t1=System.currentTimeMillis();
		//System.out.println("2) Selección de tipos de líneas "+(t1-t2)/1000);
		//System.out.println("Número de líneas de fondo "+numLineasFondo);
		if(computeFondo)
			{
			...
			if(step>1)
				{
				int []px=new int[datos.getNumFields()*2];
				int []py=new int[datos.getNumFields()*2];
				for(int i=0;i<datos.getNumFields();i++)
					{
					py[i]=(int)(margenSup+maxVar[i]);
					px[i]=(int)(margenIzq+intervaloVar*i);
					}
				int cont=datos.getNumFields();
				for(int i=(datos.getNumFields())*2-1;i>=datos.getNumFields();i--)
					{
					py[i]=(int)(margenSup+longEjeY+minVar[i-(datos.getNumFields()-1)-1]);
					px[cont++]=(int)(margenIzq+intervaloVar*(i-(datos.getNumFields())));
					}
				//gbTemp.setPaint(paleta[colorLineaOut].brighter());
				gbTemp.setPaint(new Color(220,220,220));
				gbTemp.fillPolygon(px,py,px.length);
				}
			//System.out.println("Dibujando líneas");
		...
		t2=System.currentTimeMillis();
		//System.out.println("3) Dibujado lineas de fondo "+(t2-t1)/1000);
		if(selecBic==null)
			{
			//g2.setPaint(paleta[colorLinea]);
	...
	//	System.out.println("Tarda en el draw: "+(System.nanoTime()-time));
	...
	//	System.out.println("Tarda en el draw: "+(System.nanoTime()-time));
		t1=System.currentTimeMillis();
		//System.out.println("4) Dibujado de resto de líneas "+(t1-t2)/1000);
	...
	public int getId(){
		return kernel.Configuration.PARALLEL_COORDINATES_ID;
	}
	 ...
	 */
	public void update() {
		System.out.println("Actualizando coordenadas paralelas");
		repaintAll=true;
		fitScrolls();
		this.repaint();
	}
	 ...
	 void fitScrolls()
		...
		TupleSelection selecPuntos = this.getDataLayer().getPointSelection();
		TupleSelection filterPuntos = this.getDataLayer().getFilterPoints();
		double maxLineas[]=new double[datos.getNumFields()];
		double minLineas[]=new double[datos.getNumFields()];
		for(int i=0;i<datos.getNumFields();i++)		maxLineas[i]=minLineas[i]=-111;
		...
		if(sesion.getSelectedBicluster()!=null && sesion.getSelectedBicluster().getGenes().size()>0)
			...
			for(int i=0;i<datos.getNumTuples();i++)
				...
				if(tuplaSeleccionada!=i)
					{
					if(filterPuntos==null || filterPuntos.isSelectedX(i))
						{
						if(selecPuntos!=null && selecPuntos.getNumTuples()>0 && (selecPuntos.isSelectedX(i) || selecPuntos.getElementY(i)) )
							{
							for(int j = 0; j < (datos.getNumFields()-1); j++)
			...
			if(sesion.getSelectedBicluster().getConditions().size()<sesion.getMicroarrayData().getNumConditions()-1)
				...
				for(int i=0;i<sesion.getSelectedBicluster().getConditions().size();i++)
					...
					int posCib=-1;
					//for(int j=0;j<ordenVars.length;j++)	System.out.print(ordenVars[j]+" ");
					//System.out.println();
	...
	public void updateData(){
		System.out.println("Actualizamos los datos");//ojo, debe ocurrir s'olo cuando se carga un nuevo microarray, no datos de resultados o trn
		if(sesion.getData()!=datos)
			{
			atributosIniciados = false;
			datos = sesion.getData();
			this.iniciarAtributos();
...
//			repaint();
	...
	private class GestorCambioVars implements MouseListener, MouseMotionListener{
		...
		public void mousePressed(MouseEvent e) {
			if(sesion.dataLoaded() && atributosIniciados){
			...
			System.out.println("Var seleccionada: "+varSeleccionada+" pos: "+posSeleccionada);
			}
		}
			
		public void  mouseReleased(MouseEvent e) {
			if(sesion.dataLoaded() && (ejeSeleccionado != null) && varSeleccionada!=-1)
				{
				...
				if(e.getX() < margenIzq)
					...
					nuevaPosicion = datos.getNumFields()-1;
				...
				if(posSeleccionada != nuevaPosicion){
					...
					int[] aux = new int[datos.getNumFields()];
					...
					System.out.println("Pos seleccionada "+posSeleccionada+" va a nueva pos "+nuevaPosicion);
		...
		public void mouseDragged(MouseEvent e) {
			...
			if(sesion.dataLoaded() && (ejeSeleccionado != null)){
	...
	private class GestorScrolls implements MouseListener,MouseMotionListener {
		...
		public void mouseReleased(MouseEvent e) {
			...
			if(sesion.dataLoaded()){
				...
				TupleSelection selec=new TupleSelection("","",datos.getNumTuples());
				...
				if(getDataLayer().getPointSelection()!=null)	System.out.println("Puntos seleccionados antes "+getDataLayer().getPointSelection().getNumSelected());
				...
				System.out.println("Seleccionados "+genes.size()+" genes"+" y "+conditions.size());
	...
	private class GestorCursor implements MouseMotionListener {
		...
		public void mouseMoved(MouseEvent e) {
			if(sesion.dataLoaded() && atributosIniciados && diagramaPintado){
				int zonaSelec = 2;
				...
				if(!zonaEje){
					for(int i = 0; i < (datos.getNumFields()-1); i++){
						for(int j = 0; j < datos.getNumTuples(); j++){
							if((tuplas[j][i].linea.ptSegDist(e.getPoint()) < zonaSelec)
								&& !scrollSup[i].contains(e.getPoint()) && !scrollInf[i].contains(e.getPoint())){
								zonaTupla = true;
