...
public class Graph {
	...
	public void drawHoverNode() {
		...
		if (hoverNode != null) {
			...
			// if(hoverNode!=null &&
			// !selectedNodes.containsKey(hoverNode.label))
			if (hoverNode != null
				...
				// (float)Math.floor(hoverNode.position.getX()+1),
				// (float)Math.floor(hoverNode.position.getY()-hoverNode.getHeight()+1));
	...
	public void drawHoverDualNode() {
		...
		if (hoverNode != null &&  hoverNode.getClass().toString().contains("DualNode")) {
			...
			while (itDrawMates.hasNext()) {
				...
				// if(n.clusters.size()>=bv.nodeThreshold &&
				// this.nodes.containsKey(n.label) && !nodeExcluded(n))
				if (cic > 0) {
