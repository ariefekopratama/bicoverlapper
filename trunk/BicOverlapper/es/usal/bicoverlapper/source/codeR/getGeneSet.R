...
getGenesInGeneSet=function(geneNames, environment, geneSet)
    ...
    #library(rJava)
    #.jinit("/Users/rodri/Documents/workspace/Rtests/bin/RUtils.jar")
    #ru=.jnew("utils.RUtils")
    #.jcall(ru, returnSig = "V", method="listToMatrix", gl, dim(eset)[1],length(paths))
    
    
    #.jcall(ru, returnSig = "[I", method="listToMatrixSimple", dim(eset)[1],length(paths))
    
    #ret=.jcall(ru, returnSig = "S", method="sayHello", "Rodri")
    #print(ret)
