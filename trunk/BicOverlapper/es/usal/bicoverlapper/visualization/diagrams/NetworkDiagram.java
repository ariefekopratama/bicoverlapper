 ...
 * TODO: Superslow for thousands of nodes
 *
 */
public class NetworkDiagram extends Diagram {
	...
	private Color[] paleta = {null, null, null, new Color(0,0,0,123), new Color(200,200,200,123), Color.WHITE, null, null,null};
	private String[] textoLabel = {"Selection", "Search", "Hover", "Activation edge",
								   "Inhibition edge", "Background", "Low expression color", "Average expression color", "High expression color"};
	...
	/**
	 * If it's true and there's expression data loaded, the network is mapped to the expression of every sample, iteratively, in an animation
	 */
	...
	void create(NetworkData trnd)
		...
		//--------- color
		int[] ePalette = new int[]{ paleta[NetworkDiagram.colorActivation].getRGB(), ColorLib.gray(100,123), paleta[NetworkDiagram.colorInhibition].getRGB()};
	...
	//    VisualItem.STROKECOLOR, ColorLib.gray(200));
		...
		int paletteTemp[]=ColorLib.getInterpolatedPalette(255,paleta[NetworkDiagram.lowExpColor].getRGB(), paleta[NetworkDiagram.avgExpColor].getRGB());
		int paletteTemp2[]=ColorLib.getInterpolatedPalette(255,paleta[NetworkDiagram.avgExpColor].getRGB(), paleta[NetworkDiagram.highExpColor].getRGB());
		...
		ActionList layout = new ActionList(10000);//Mejor, se queda quieto al fin
...
//        animate.add(new PolarLocationAnimator("graph.nodes", linear));
        ...
        if(d==null)		
    		...
    		currentGenes=new NetworkFocusControl(sesion,"filter", Visualization.FOCUS_ITEMS, v);
        ...
        JPanel search = sq.createSearchPanel();
		...
		if(v!=null && sesion!=null && sesion.getSelectedBicluster()!=null)
			...
			if(selc!=null && selc.size()>=1)
				...
				while(it.hasNext())
					...
					double exp=0;
					for(Object i:selc)//Compute average of expression levels for the condition
						exp+=sesion.getMicroarrayData().getExpressionAt(item.getInt("id"), (Integer)i);
					exp/=selc.size();
	...
	public void changeLabels()
		...
		while(it.hasNext())
			{
			VisualItem item=(VisualItem)it.next();
			item.setString("name", sesion.getMicroarrayData().getRowLabel(item.getInt("id")));
			}
	...
	public void configure(){
		if(!configurando){
			...
			if(this.getPanelParametros()==null)	
				this.setPanelParametros(new NetworkParameterConfigurationPanel(fdl));
		...
		//	this.initPanelConfig(panelColor, null, null, panelBotones);
			this.initPanelConfig(panelColor, null, this.getPanelParametros(), panelBotones);
	...
	public void endConfig(){
		...
		if(((NetworkParameterConfigurationPanel)this.getPanelParametros()).getAnimateExpression().isSelected())
		 	{
			if(sesion.getMicroarrayData()!=null)
				{
				if(p==null)
...
*/
	...
	private class ExpressionColorThread extends Thread	
       ...
       /**
          * Iterated over each sample coloring the network in an animation until it is stopped
          */
         public void run()
         	{
        	if(sesion.getMicroarrayData()==null)	return;
        	try{
        	Iterator it=v.items("graph.nodes");
        	color.remove(noFillColor);
			color.add(expressionColor);
			...
			if(sesion!=null && sesion.getSelectedBicluster()!=null)	selg=sesion.getSelectedGenesBicluster();
        	...
        	while(!stop)
				...
				for(int i=0; i<sesion.getMicroarrayData().getNumConditions(); i++)
					{
					title.setText(sesion.getMicroarrayData().getColumnLabel(i));
