...
public class MicroarrayData {
	...
	private Table convert(Table mat, int skipRows, int skipColumns) {
		...
		for (int i = 0; i < numConditions; i++) {
			...
			minCols[i] = maxCols[i] = new Double(mat.getString(0 + skipRows, i
					+ skipColumns)).doubleValue();
		}
		...
		for (int i = 0; i < numGenes; i++) {
			for (int j = 0; j < numConditions; j++) {
				...
				try {
					matrix[i][j] = new Double(mat.getString(i + skipRows, j
							+ skipColumns)).doubleValue();
				} catch (NumberFormatException e) {
					JOptionPane
							.showMessageDialog(
	...
	private class AnnotationStartThread extends Thread {
		 ...
		 * 
		 * @param exp
		 *            REXP with the console into which to call for the install
		 *            methods
		 * @param lib
	...
	public void getGOTermsHypergeometric(LinkedList<Integer> genes,
		...
		if (location != null)
			hpm = new HypergeometricTestProgressMonitor(location);
		else
			hpm = new HypergeometricTestProgressMonitor(new Point(0, 0));
		hpm.setTask(ht);
		hpm.run();
		if (gr != null) {
			geneRequester = gr;
			...
			SwingWorker<ArrayList<GOTerm>, Void>// implements Runnable
	{
		...
		public ArrayList<GOTerm> getGOTermsHypergeometric(
			...
			/*
			 * for(int i=0;i<geneNames.length;i++) { if(i<geneNames.length-1)
			 * universe=universe.concat(geneNames[i]+"\", \""); else
			 * universe=universe.concat(geneNames[i]+"\")"); }
			 */
			...
			SwingWorker<ArrayList<GeneAnnotation>, Void>// implements Runnable
	{
		...
		public ArrayList<GeneAnnotation> getMultipleGeneAnnotationsR() {
			...
			if (searchGO) {
				...
				if (exp != null) {
					...
					if (goids != null) {
						...
						if (exp == null)
							System.err
									.println("Error getting GO terms by ID with R");
			...
			for (int g = 0; g < genes.length; g++) {
				...
				// if(searchGO && go!=null && go.at(ga.id).asList()==null) //If
				// searched and NOT found, we still add a GOTerms structure so
				// we know it has already been searched for
				// if(searchGO && go!=null && goids==null) //If searched and NOT
				// found, we still add a GOTerms structure so we know it has
	...
	public class LoadTask extends SwingWorker<Integer, Void> {
		...
		public int loadFromFile() {
			...
			Table levelsi = null;
			try {
				levelsi = tr.readTable(new FileInputStream(path)); // esto es lo
																	// que mas
																	// peso
																	// tiene en
																	// tiempo
		...
		for (int i = 0; i < numGenes; i++) {
			...
			for (int j = 0; j < numConditions; j++) {
				...
				if (!lowEFV.equals("none333")
						&& ((lowEFV.equals("rest") && !efvsL[j].equals(highEFV)) || efvsL[j]
								.equals(lowEFV)))
