	...
	private void initGUI() {
		try {
				...
				}
			{
				...
				{
					jList1Model = new DefaultComboBoxModel(efs.toArray(new String[0]));
					jList1 = new JList();
			...
			}
			{
				...
				{
					jList2Model = new DefaultComboBoxModel(efs.toArray(new String[0]));
					jList2 = new JList();
...

			{
				...
				jButton1.addActionListener(new java.awt.event.ActionListener() {
					public void actionPerformed(java.awt.event.ActionEvent e) {
						String highEF=null;
						if(jList1.getSelectedIndex()==0)	highEF="rest";
						else
							{
							for(int i=jList1.getSelectedIndex()-1;i>=0;i--)
								{
								if(!jList1Model.getElementAt(i).toString().startsWith(" "))	
								 {highEF=jList1Model.getElementAt(i).toString(); break;}
								}
							}
						String lowEF=null;
						if(jList2.getSelectedIndex()==0)	lowEF="rest";
						else
							{
							for(int i=jList2.getSelectedIndex()-1;i>=0;i--)
								{
								if(!jList2Model.getElementAt(i).toString().startsWith(" "))	
										{lowEF=jList2Model.getElementAt(i).toString(); break;}
								}
							}
						int sdh=3-jComboBox1.getSelectedIndex();
						int sdl=3-jComboBox2.getSelectedIndex();
					if(jCheckBox2.isSelected() && jCheckBox1.isSelected())
						session.setSelection(jList1.getSelectedValue().toString().trim(), highEF, sdh, jList2.getSelectedValue().toString().trim(), lowEF, sdl);
					else
						{
						if(jCheckBox1.isSelected())	session.setSelection(jList1.getSelectedValue().toString().trim(), highEF, sdh, "none333", "none333", sdl);
						if(jCheckBox2.isSelected())	session.setSelection("none333",  "none333", sdh, jList2.getSelectedValue().toString().trim(), lowEF, sdl);
						}
					setVisible(false);
					}
				});
