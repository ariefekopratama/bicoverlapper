...
public class FileMenuManager implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		try{
			...
			pathReader=new BufferedReader(new FileReader(getPath("data\\path.txt")));
		...
		{
		JFileChooser selecFile = new JFileChooser();
		selecFile.addChoosableFileFilter(new es.usal.bicoverlapper.data.files.TRNFilter());
		selecFile.setCurrentDirectory(new File(defaultPath));
		//selecFile.setLocale(new Locale("en"));
		...
		{
		System.out.println("default path es " +defaultPath);
		JFileChooser selecFile = new JFileChooser();
		selecFile.addChoosableFileFilter(new es.usal.bicoverlapper.data.files.MicroarrayFilter());
		selecFile.setCurrentDirectory(new File(defaultPath));
		//selecFile.setLocale(new Locale("en"));
		int returnval = selecFile.showDialog(
		...
		if(returnval == JFileChooser.APPROVE_OPTION) {
			...
			System.out.println("Abrimos nuevo perfil por Biclusters");
			JDesktopPane desktop = new JDesktopPane();
			desktop.setName(fichero.getName());
			boolean addVista=false;
			if(this.ventana.getActiveWorkDesktop()!=null)
	...
	//---------------- ABRIR XML DE PELICULAS ---------------------
	/*
	else if(e.getActionCommand().equals(Translator.instance.menuLabels.getString("s4")))
	{
	JFileChooser selecFile = new JFileChooser();
	selecFile.addChoosableFileFilter(new ficheros.TRNFilter());
	
	int returnval = selecFile.showDialog(
			(Component)e.getSource(),Translator.instance.menuLabels.getString("s4"));
	...
	if(returnval == JFileChooser.APPROVE_OPTION) {
		...
		JDesktopPane desktop = new JDesktopPane();
		desktop.setName(fichero.getName());
		sesion = new Sesion(desktop);

		try {
			leerPelicula(path, fichero.getName(),sesion);
		} catch (FileNotFoundException e1) {
			JOptionPane.showMessageDialog(null,
					Translator.instance.warningLabels.getString("s3")+" "+fichero.getName(),
                    Translator.instance.warningLabels.getString("s2"),JOptionPane.ERROR_MESSAGE);
		...
		// Actualizar las ventanas activas				
		sesion.ficheroCargado();
		sesion.setDatosPeliculaCargados(true);
		sesion.actualizarDatos();
		ventana.addVista(new Vista(desktop,sesion));
				...
				&& ventana.isActiveWorkDesktop())
		{
			...
			if(returnval == JFileChooser.APPROVE_OPTION){
				...
				es.usal.bicoverlapper.kernel.configuration.ConfigFileHandler.loadConfiguration(sesion,fichero);
			}			
		}
				...
				&& ventana.isActiveWorkDesktop())
		{
			...
			if(returnval == JFileChooser.APPROVE_OPTION){
				File fichero = selecFile.getSelectedFile();
				es.usal.bicoverlapper.kernel.configuration.ConfigFileHandler.saveConfiguration(config, fichero);
			}			
		}	
	...
	//	try {
			...
			//System.out.println("tiempo en new MicroarrayData: "+(t2-t1)/1000);
			...
			System.out.println("tiempo en leerFichero: "+(t1-t2)/1000);
	...
	private void leerTRN(String path, File fichero,Session sesion) throws FileNotFoundException,IOException 
		...
		if(variable.contains("GeneNetwork") || linea2.contains("graphml"))
			...
			in.close();
			in2.close();
			//System.out.println("Es grafo XML de Syntren: "+path+"\\"+fichero.getName());
